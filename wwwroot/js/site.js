...
function atualizarMenu() {
    const aberto = innerWidth <= 980
        ? document.body.classList.contains('menu-aberto')
        : !document.body.classList.contains('sidebar-recolhida');
    menuToggle?.setAttribute('aria-expanded', String(aberto));
    if (sidebar) sidebar.inert = !aberto;
}
document.querySelectorAll('.menu-toggle,.menu-overlay').forEach(botao => botao.addEventListener('click', () => {
    document.body.classList.toggle(innerWidth <= 980 ? 'menu-aberto' : 'sidebar-recolhida');
    atualizarMenu();
}));
...
document.addEventListener('app:navigated', atualizarMenu);
atualizarMenu();
...
(() => {
  ...
  const internal = url => url.origin === location.origin &&
    !/^\/(?:Conta\/Entrar|Conta\/Cadastrar|Conta\/EsqueciSenha|Conta\/Sair)(?:\/|$)/i.test(url.pathname);
